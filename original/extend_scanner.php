...
class ExtendScanner extends Scanner
{
	public $outputNum;

	function __construct($file_name, $scan_functions, $info_functions, $source_functions)
	{
		parent::__construct($file_name, $scan_functions, $info_functions, $source_functions);
		$this->outputNum = 0;
	}
	...
	//EDIT
	//Add param $parse_pos
	function scan_parameter($mainparent, $parent, $var_token, $var_keys=array(), $last_token_id, $var_declares, $var_declares_global=array(), $userinput, $F_SECURES=array(), $return_scan=false, $ignore_securing=false, $secured=false, $parse_pos = -1)
	{
	//ENDEDIT
...
#echo "<br>var:".$var_name; echo " varkeys:";print_r($var_token[3]); echo " declarekeys:";print_r($var_declare->array_keys); echo " diff:"; print_r($array_key_diff); echo " |||";
		...
		else if(SCAN_REGISTER_GLOBALS && $var_token[0] === T_VARIABLE && !in_array($var_name, Sources::$V_USERINPUT) && (!$this->in_function || (in_array($var_name, $this->put_in_global_scope) && !in_array($var_name, $this->function_obj->parameters))) && empty($secured))
		{
			...
			for ($i = $parse_pos; $this->tokens[$i] !== ';'; $i++) {
				if (is_array($this->tokens[$i]) && in_array($this->tokens[$i][1], $this->function_obj->parameters)) {
					$userinput = 2;
				}
			}
			//ENDEDIT
	...
	//EDIT
	//add $parse_pos
	function variable_scan($i, $offset, $category, $title, $parse_pos = -1)
	{
			...
			if( is_array($this->tokens[$i]) )
			{
				...
				*************************/
				if($token_name === T_VARIABLE)
				{
					...
					{
						$this->variable_scan($i, 0, 'eval', 'Userinput is used as dynamic function name. Arbitrary functions may be called.', $i);
					}
					// $$var =
					else if( ($this->tokens[$i-1] === '$' || ($this->tokens[$i-1] === '{' && $this->tokens[$i-2] === '$')) && ($this->tokens[$i+1] === '=' || in_array($this->tokens[$i+1][0], Tokens::$T_ASSIGNMENT)) )
					{
						$this->variable_scan($i, $this->tokens[$i-1] === '{' ? 2 : 1, 'extract', 'Userinput is used to build the variable name. Arbitrary variables may be overwritten/initialized which may lead to further vulnerabilities.', $i);
					//ENDEDIT
					}
					...
					if( in_array($token_value, Sources::$V_USERINPUT) )
					{
						...
						//EDIT
						if ($this->tokens[$i-2][1][0] !== '$') {
							$called_class = strtolower(trace_param($this->tokens[$i-4], $this, $i));
							if (!empty($called_class)) {
								$called_class_property = $called_class.'::'.$this->tokens[$i-2][1];
								$GLOBALS['object_flag'][$called_class_property] = true;
							}
						}
						else {
							$called_method = strtolower(trace_param($this->tokens[$i-2], $this, $i));
							$object_thing = $called_method;
						}
						//ENDEDIT
				...
				|| (in_array($token_name, Tokens::$T_XSS) && ($_POST['vector'] == 'client' || $_POST['vector'] == 'xss' || $_POST['vector'] == 'all')) )
				{
					...
					if($token_name === T_STRING && $this->tokens[$i+1] === '(')
					{
						...
						else
						{
							...
							if($this->tokens[$i-1][0] === T_OBJECT_OPERATOR)
							{
								...
								//EDIT
								//Specify class name
								$result = $this->var_declares_global[$this->tokens[$i-2][1]][0]->tokens[2];
								$search_class = trace_param($result, $this, $i);

								$specify_class = $this->class_vars[$search_class];
								if (empty($specify_class)) {
									$specify_class = strtolower($this->tokens[$i-2][1]);
								}
								$specify_class_method = $specify_class.'::'.$token_value;
								...
								//Judge whether function param orinate from user input or not
								$param = $this->tokens[$i+2];
								$search_input = trace_param($param, $this);
								...
								echo " → ";
								var_dump($GLOBALS['vul_func'][$specify_class_method]['userinput']);var_dump($this->tokens[$i][2]);
								echo '</div>';
					...
					*************************/
					...
					//EDIT
					if ($GLOBALS['verbosity'] != 5 && $this->function_obj->name === '__construct') {
						$enbrace = 1;
						for ($j = -2; $enbrace !== 0; $j++) {
							if ($this->tokens[$i + $j] === '{') {
								$enbrace++;
							}
							else if ($this->tokens[$i + $j] === '}') {
								$enbrace--;
							}
							if (in_array($this->tokens[$i + $j][1], Sources::$V_USERINPUT)) {
								$property = trace_param($this->tokens[$i + $j], $this, $i + $j);
								$GLOBALS['object_flag'][strtolower($this->class_name)."::".$this->tokens[$i + $j - 2][1]] = true;
							}

							//CONTINUE
							//In case __construct($param)
							if (true) {
							}
						}
					}
					else if (!empty($this->function_obj->name) && !empty($this->class_name)){
						// function getParam() {
						// 		return $this->object
						// }
						//
						// alias
						// $GLOBALS['object_flag']["hoge::getparam"]= &$GLOBALS['object_flag']["hoge::object"]
						//TODO: Enforcement
						$enbrace = 1;
						...
						for ($j = -4; $enbrace !== 0; $j++) {
							...
							if ($this->tokens[$i + $j + 1][1] === 'return') {
								$property = trace_param($this->tokens[$i + $j + 4], $this, $i + $j + 4);
								$GLOBALS['object_flag'][strtolower($this->class_name)."::".strtolower($this->function_obj->name)]
									= &$GLOBALS['object_flag'][strtolower($this->class_name)."::".strtolower($property)];
							}
						}
					...
					if(isset($this->scan_functions[$token_value])&& $GLOBALS['verbosity'] != 5) {
						//EDIT
						if (!empty($this->class_name) && !empty($this->function_obj->name)) {
							$vul_function = strtolower($this->class_name)."::".strtolower($this->function_obj->name);
						}
						else {
							for ($j = 0; $this->tokens[$i+$j] !== ';'; $j++) {
								if (!empty($this->tokens[$i+$j]) && $this->tokens[$i+$j][1][0] === '$') {
									//$result = trace_param($this->tokens[$i+$j], $this, $i+$j);
									$analysis_tokens = $this->var_declares_global[$this->tokens[$i+$j][1]][0]->tokens;
									for ($k = 0; $k < count($analysis_tokens); $k++) {
										if ($analysis_tokens[$k][1] === '->') {
											$vul_function = trace_param($analysis_tokens[$k-1],$this, $k-1)."::".$analysis_tokens[$k+1][1];
										}
									}
						...
						$GLOBALS['vul_func'][$vul_function]['flag'] = true;
						$GLOBALS['vul_func'][$vul_function]['function'] = $token_value;
						$GLOBALS['vul_func'][$vul_function]['line'] = $line_nr;
						...
						//Process in case property of object is called by echo and so on
						for ($j = 0; $this->tokens[$i+$j] !== ';'; $j++) {
							if (is_array($this->tokens[$i+$j]) && $this->tokens[$i+$j][1][0] === '$') {
								$result = trace_param($this->tokens[$i+$j], $this, $i+$j);
								$called_property = $this->tokens[$i+$j+2][1];
								if (!empty($called_property) && !empty($result)) {
						...
						if($userinput === true) {
							$GLOBALS['vul_func'][$vul_function]['userinput'] = $userinput;
						}
						else if($userinput === 2) {
							$GLOBALS['vul_func'][$vul_function]['userinput'] = $userinput;
						}
						else if($userinput === false && $GLOBALS['vul_func'][$vul_function]['userinput'] !== true && $GLOBALS['vul_func'][$vul_function]['userinput'] !== 2) {
							$GLOBALS['vul_func'][$vul_function]['userinput'] = $userinput;
						}
						else {
						}
					...
					&& (empty($class) || (($this->in_function && is_array($function_obj->parameters) && in_array($classvar, $function_obj->parameters)) || @in_array($token_value, $this->vuln_classes[$class]))) )
					{
						if(!$this->already_scanned($i))
						{
							...
							if( ($has_vuln_parameters && ($parameter_has_userinput || $parameter_func_depend)) || $GLOBALS['verbosity'] == 4 || isset($this->scan_functions[$token_value][3]) )
							{
								...
								//EDIT
								$trace_param = array_slice($this->tokens,$vulnstart,$c+$vulnadd);

								if($trace_param[1][1] == '->') {
									$trace_class = $trace_param[0][1];
									$trace_func = $trace_param[2][1];

									$trace = array();
									$trace['class'] = $trace_class;
									$trace['function'] = $trace_func;
									$GLOBALS['candidate'][] = $trace;
									//$search_func = $trace_class.'::'.$trace_func;
								}
								//ENDEDIT
